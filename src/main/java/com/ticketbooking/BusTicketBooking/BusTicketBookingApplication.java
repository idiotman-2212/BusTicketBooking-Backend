package com.ticketbooking.BusTicketBooking;

import com.ticketbooking.BusTicketBooking.entity.*;
...
import com.ticketbooking.BusTicketBooking.repository.*;
		...
		for (int i = 1; i <= 20; i++) {
			users.add(
					User.builder().username("user" + i).password(passwordEncoder.encode("123456")).firstName("Mr").lastName("User " + i)
							...
							.gender(false).address("blabla").isActive(true).build()
			);
		}
		users.add(User.builder().username("admin").password(passwordEncoder.encode("123456")).firstName("Diễn").lastName("Châu")
				.email("abclsdjf23@gmail.com").phone("0326917158").dob(LocalDate.of(2002, 12, 22))
				.gender(true).address("blabla").isActive(true).build());
